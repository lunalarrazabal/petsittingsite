...
export async function POST(request: Request) {
  ...
  if (!name?.trim() || !email?.trim() || !phone?.trim() || !petName?.trim() || !petType || !servicesRaw || !date) {
  ...
  const serviceListHtml = serviceLabels
    .map((label) => `<li style="padding: 3px 0; color: #111827;">${label}</li>`)
    .join('');
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #fff; padding: 24px; border: 1px solid #d1fae5; border-top: none; border-radius: 0 0 12px 12px;">
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
              <tr>
                <td style="padding: 6px 0; color: #6b7280; width: 140px; vertical-align: top;">Services</td>
